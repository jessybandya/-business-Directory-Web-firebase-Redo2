...
function Allpostsjobs() {
  let { uid } = useParams();
   ...
   useEffect(() => {
    db.collection('businesses').where("type","==","job").orderBy("timestamp", "desc").onSnapshot(snapshot => {
...
<Form inline className="Search-header">
      ...
      <FormGroup>
      <p className="card-text"><small className="text-muted">Jobs near me</small></p>
    ...
    </Form>
{
    posts.map(({ id, post }) => (
        ...
        ownerEmail={post.companyEmail} 
        timestamp={post.timestamp}
        web={post.web}
        ownerId={post.ownerId} 
        ownerUsername={post.ownerUsername}
        ownerphotoURL={post.ownerphotoURL}
        phone={post.phoneCompany}
